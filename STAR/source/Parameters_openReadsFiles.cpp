...
//void Parameters::openReadsFiles(JNIEnv * env, jobjectArray r1, jobjectArray r2)
//{
//    if (readFilesCommandString=="") {//read from file
//        for (uint ii=0;ii<readFilesIn.size();ii++) {//open readIn files
//            readFilesCommandPID[ii]=0;//no command process IDs
...
//            if (ii == 0) {
//                inOut->logMain << "\n   R2 starting " <<" :\n";
//                jsize fSize = env->GetArrayLength(r2);
//                for (int i = 0; i < fSize; i++) {
//                    jstring chardata = (jstring)env->GetObjectArrayElement(r2, i);
//
////                     char*   rtn   =   NULL;
////                    jclass   clsstring   =   env->FindClass("java/lang/String");
////                    jstring   strencode   =   env->NewStringUTF("GB2312");
////                    jmethodID   mid   =   env->GetMethodID(clsstring,   "getBytes",   "(Ljava/lang/String;)[B");
////                    jbyteArray   barr=   (jbyteArray)env->CallObjectMethod(chardata,mid,strencode);
...
////                    free(rtn);
//
////                    inOut->logMain << stemp <<endl<<flush;
////                    inOut->logMain << (std::string)env->GetStringUTFChars(chardata, NULL) << endl << flush;
//                    inOut->readIn[0] << (std::string)env->GetStringUTFChars(chardata, NULL);
//                }
//
//                inOut->logMain << "\n   R2 finishing " <<"\n";
//            } else {
//
//                inOut->logMain << "\n   R1 starting " <<" :\n";
//                jsize fSize = env->GetArrayLength(r1);
//                for (int i = 0; i < fSize; i++) {
//                    jstring chardata = (jstring)env->GetObjectArrayElement(r1, i);
//
////                    char*   rtn   =   NULL;
////                    jclass   clsstring   =   env->FindClass("java/lang/String");
////                    jstring   strencode   =   env->NewStringUTF("GB2312");
...
////                    inOut->logMain << stemp <<endl<<flush;
////                    inOut->logMain << (std::string)env->GetStringUTFChars(chardata, NULL) << endl << flush;
//                    inOut->readIn[1] << (std::string)env->GetStringUTFChars(chardata, NULL);
//                }
//                inOut->logMain << "\n   R1 finishing " <<"\n";
//            }
//            if (inOut->readIn[ii].fail()) {
