...
// stringstream GenomeIn;
ifstream GenomeIn, SAin, SAiIn;

void Genome::genomeLoad(JNIEnv * env, jobjectArray inGenome) {
    int inGenomeN = env->GetArrayLength(inGenome);
    // int inSAiN = env->GetArrayLength(inSAi);
    jarray jniarray;
    for (int i = 0; i < inGenomeN; i++) {
        jniarray = (jbyteArray)env->GetObjectArrayElement(inGenome, i);
        jint cols = env->GetArrayLength(jniarray);
        jbyte* coldata = env->GetByteArrayElements((jbyteArray)jniarray, nullptr);
        // GenomeIn << coldata[i];
    }
    // for (int i = 0; i < inSAiN; i++) {
    //     jniarray = (jbyteArray)env->GetObjectArrayElement(inSAi, i);
    //     jint cols = env->GetArrayLength(jniarray);
    //     jbyte* coldata = env->GetByteArrayElements((jbyteArray)jniarray, nullptr);
