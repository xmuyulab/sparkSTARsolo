...
uint Genome::OpenStream(string name, ifstream & stream, uint size)
{
    ...
    P.inOut->logMain << name << ": size given as a parameter = " << stream.gcount() << " (use for test)" <<"\n";
    if (size>0) {
        P.inOut->logMain << name << ": size given as a parameter = " << size << " (use for test)" <<"\n";
    } else {
...
uint Genome::OpenStream(string name, stringstream & stream, uint size)
{
    // stream.open((pGe.gDir+ "/" +name).c_str(), ios::binary);
    if (!stream.good()) {
        ostringstream errOut;
