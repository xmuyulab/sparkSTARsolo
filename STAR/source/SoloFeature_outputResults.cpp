...
{    
    ...
    */
    ...
    switch (featureType) {
        ...
        {
            // pSolo.outFileNames[1] = features.tsv 
    ...
    if (cellFilterYes) {//filtered cells
        for (uint32 icb=0; icb<nCB; icb++) {
            if (filteredCells.filtVecBool[icb]) {
                ...
                filteredCells.barcodes.push_back(pSolo.cbWLstr[indCB[icb]]);
...
//        if (cellFilterYes) {
//            P.inOut->out_Matrix.push_back(Trans.geID[featuresNumber - 1] + "_" + (Trans.geName[featuresNumber - 1].empty() ? Trans.geID[featuresNumber -1] : Trans.geName[featuresNumber - 1]) + "_" + pSolo.cbWLstr[(cellFilterYes ? filteredCells.nCells : pSolo.cbWLsize) -1] + "_" + to_string(nCellGeneEntries));
//        } else {
//            countMatrixStream << featuresNumber <<' '<< (cellFilterYes ? filteredCells.nCells : pSolo.cbWLsize) <<' '<< nCellGeneEntries << '\n';
//        }
        uint32  cbInd1=0;
        for (uint32 icb=0; icb<nCB; icb++) {
            ...
            for (uint32 ig=0; ig<nGenePerCB[icb]; ig++) {
                ...
                if (cellFilterYes) {
                    P.inOut->out_Matrix.push_back(Trans.geID[countCellGeneUMI[indG1]] + "_" + (Trans.geName[countCellGeneUMI[indG1]].empty() ? Trans.geID[countCellGeneUMI[indG1]] : Trans.geName[countCellGeneUMI[indG1]]) + '_' +  filteredCells.barcodes[cbInd1 -1] + '_' + to_string(countCellGeneUMI[indG1+iCol]));
                } else {
