...
#include "jni.h"
#include "com_github_xmuyulab_sparkStarSolo_jstar_HeadFile.h"
#include "com_github_xmuyulab_sparkStarSolo_adapter_SoloInit.h"
#include <iostream>

JNIEXPORT jobjectArray JNICALL JNICALL Java_com_github_xmuyulab_sparkStarSolo_adapter_SoloInit_runSolo(JNIEnv * env, jobject , jobjectArray commandLine, jobjectArray r1, jobjectArray r2, jobjectArray inGenome) {

    jarray singleArgIn;
    jchar* tmpCol;
    int argInN = env->GetArrayLength(commandLine);
    jint col;
    char** argIn = new char*[argInN];
    for (int i = 0; i < argInN; i++) {
        singleArgIn = (jarray)env->GetObjectArrayElement(commandLine, i);
        col = env->GetArrayLength(singleArgIn);
        tmpCol = env->GetCharArrayElements((jcharArray)singleArgIn, 0);
    ...
    P.inputParameters(argInN, argIn, env, r1, r2);
    ...
    ///////////////////////////////// Genome
    Genome genomeMain (P, P.pGe);
    P.inOut->logMain << "we can load Genome in jniSolo.cpp 105"<<endl<<flush;
    genomeMain.genomeLoad(env, inGenome);
...
/////////////////////////////////////////////////////////////////////////////////////////////////START
    ...
    *P.inOut->logStdOut << timeMonthDayTime(g_statsAll.timeStartMap) << " ..... started mapping\n" <<flush;
    ...
    if (P.runRestart.type!=1) {
        *P.inOut->logStdOut << timeMonthDayTime(g_statsAll.timeStartMap) << " ..... STAR.cpp 161\n" <<flush;
        mapThreadsSpawn(P, RAchunk);
        *P.inOut->logStdOut << timeMonthDayTime(g_statsAll.timeStartMap) << " ..... STAR.cpp 163\n" <<flush;
    }
    ...
    *P.inOut->logStdOut << timeMonthDayTime(g_statsAll.timeFinish) << " ..... finished successfully\n" <<flush;
    ...
    long v_size = P.inOut->out_Matrix.size();
    jobjectArray res = (jobjectArray)env->NewObjectArray(v_size, env->FindClass("java/lang/String"), env->NewStringUTF(""));
    *P.inOut->logStdOut << "v_size is " << v_size << " \n" <<flush;
    for (long oi = 0; oi < v_size; oi++) {
        env->SetObjectArrayElement(res, oi, env->NewStringUTF(((char*)P.inOut->out_Matrix[oi].c_str())));
    }
